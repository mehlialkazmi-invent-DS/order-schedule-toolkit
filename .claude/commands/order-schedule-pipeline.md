...
**Real ticket pattern (TBRCS-757) — mistake made and corrected:** new order group `KNG_60_PAULO SOLARI SLIM DENIM`, no Excel attachment. Ticket description: "Order Schedule: Monthly, Last Fiscal Monday of each Month … Ex: (8/24/26, 9/28/26, 10/26/26, 11/23/26, 12/28/26, 1/25/27)" — the **same** "last Monday of every fiscal month" cadence as TBRCS-753's reference group `KNG_W0_CBC_FLT`. **Mistake:** built the `--manual-csv` with **all six** example dates (`order_frequency` 91 each) because a broad search for *other* `order_frequency == 91` groups turned up several with **6** rows (e.g. `JAB_40_COLEHAAN_MONTHLY_FLT`) — 6 rows went into prod (delta +6). **This was wrong.** The six dates span **two fiscal quarters** (`8/24`/`9/28`/`10/26` = one quarter's 4‑5‑4 months; `11/23`/`12/28`/`1/25` = the next quarter, same pattern repeating). Checking the *actual* matching-pattern precedent — `KNG_W0_CBC_FLT` itself, the group TBRCS-753 explicitly named as "same calendar" — shows it carries only **3 rows** (one quarter), not 6. **Fix:** re-ran with only the **first three** dates; prod delta corrected to **+3**. **Lesson:** for a "first/last `<weekday>` of every fiscal month" cadence, submit **exactly 3 rows** (one quarter) regardless of how many illustrative months the ticket lists — `order_frequency` **91** already tells the system to repeat every quarter, so a 2nd quarter's worth of rows is redundant, not "more correct." Do **not** let a broad, superficially-similar precedent search (any group with the same `order_frequency`) override the actual matching-cadence reference group or the documented row-count rule ([Fiscal cadence patterns](#fiscal-cadence-patterns-planner-intent)) — other groups with more rows may encode a different cadence entirely (e.g. `_TWICEPERSEASON_`/`_THRICEPERSEASON_` naming, or genuinely semi-annual `order_frequency` 182) and are not evidence for this pattern's row count.

**Real ticket pattern (TBRCS-758) — toolkit gap found and worked around:** two D40 order groups revised in one workbook, same title-row layout as TBRCS-685. Group 1 (`JAB_40_JA_SOIREE OXFORD FORMAL SHOE_BLACK_FLT`): "1st week of every month" — 3 consecutive fiscal months (9/3, 10/8, 11/5/2026), civil gaps 35/28 (non-uniform) — merge correctly kept all 3 rows as submitted, `order_frequency` 91. Group 2 (`JAB_40_JAB_BASIC LEATHER SNEAKER_FLT`): "1st week of every **other** month (56 days)" per the ticket text and the `OrdReview_DIV40_56D_THU_INT` review-calendar name — submitted dates were 9/3, 11/5/2026, 1/7/2027, correctly computed as the true "first Thursday of every other fiscal month" (verified against `fiscal_cal`: skips M202609/M202611, landing on M202608/M202610/M202612), but the **civil** gaps between them are a **uniform 63 days** (28+35, since one of the two skipped-over fiscal months in each pair happens to be a 5-week month) — not the "56 days" the ticket text and naming imply (that figure is a nominal 2×28-day estimate, same as how `28D` naming for group 1 doesn't mean every gap is literally 28 days; see [Fiscal months vs civil dates](#fiscal-months-vs-civil-dates-454)). The submitter also typed `order_frequency` **91** on every row of both groups (apparent copy-paste from group 1's template row), even though 91 is functionally wrong for group 2: replaying group 2's 3 anchors (spaced 2 fiscal months apart) forward with a 91-day (1-quarter) repeat lands on **all three** month-residues mod 3, which would eventually place an order in nearly every month — not every other month. The correct frequency for a 3-anchor, stride-2 ("every other fiscal month") pattern is **182** (2-quarter repeat), which tiles exactly the intended alternating months forever — matching [Fiscal cadence patterns](#fiscal-cadence-patterns-planner-intent)'s "Semi-annual in-half-year, every other fiscal month" row. **Toolkit gap:** `build_fiscal_merge_from_submission`'s uniform-gap branch only special-cases exactly **91-multiple** (seed-only) and exactly **28** (expand-to-3) uniform gaps; a uniform **63-day** gap (or any other value) falls through to the generic "default fiscal expand from earliest row," which silently reinterpreted group 2 as **consecutive** months (9/3, 10/8, 11/5 — duplicating group 1's dates and dropping the submitted 1/7/2027 row) rather than preserving the stride-2 dates. **Fix used (no code change):** re-ran `jira_input.py --manual-csv` with group 2's `order_frequency` corrected from 91 → 182 (dates unchanged) — with the same, unmodified merge script, this routes to `generate_fiscal_calendar_schedule`'s existing `stride = output_frequency // 91` logic (stride 2 for 182), which correctly reproduces the submitted every-other-month dates from `fiscal_cal`. **Lesson:** when a fiscal pair's civil gaps are uniform but not 91-multiple or exactly 28 (e.g. 56/63/70-day "every other month" style spacing), do not trust the merge tool's default expansion — verify against `fiscal_cal` directly (as in [Fiscal months vs civil dates](#fiscal-months-vs-civil-dates-454)) and, if the submitted `order_frequency` doesn't match the actual stride implied by the dates, correct it via `--manual-csv` (91→182 for stride-2, etc.) rather than accepting either the literal submitted frequency or the tool's silent fallback.
...
|-----------------|-----------|---------------------------|---------------------------|
| **First or last week/weekday of every fiscal month** ("first Wednesday of each month", "last Monday of every fiscal month", etc.) inside **one quarter** (capture **4‑5‑4** week rhythm) | **3** — one quarter only, even if the planner's example text lists more months | **91** on each row (fiscal cadence flag, not “91 civil days apart”) | First (or last) anchor weekday in each of **3 consecutive** fiscal months (e.g. `6/3`, `7/8`, `8/5`). Civil gaps may be **uneven** (35 / 28) — still **3 rows**. `order_frequency` **91** already tells the system the pattern repeats every quarter — do **not** submit a 2nd, 3rd, … quarter's worth of rows just because the ticket illustrates more months (see [TBRCS-757](#excel-layout-title-row--header-detection)). |
...
| **Anything else** (e.g. **56 / 63 / 70** — "every other fiscal month") | Stride-2 (or higher) in-quarter/half-year pattern; the exact day count varies per pair because fiscal months are 4 or 5 weeks (e.g. 28+35=63, not the nominal "56") | Toolkit has **no dedicated rule** for this — falls through to the generic default expand, which **silently collapses to consecutive months and discards the intended stride** (TBRCS-758). Verify against `fiscal_cal`; if the submitted `order_frequency` doesn't match the real stride, correct it via `--manual-csv` to the matching multiple of 91 (e.g. **182** for stride 2) so `generate_fiscal_calendar_schedule`'s existing stride logic reproduces the dates correctly — do not trust the tool's fallback output for this case |
...
| Ticket lists **more than 3** illustrative dates for a "first/last `<weekday>` of every fiscal month" cadence (e.g. 6 dates = 2 quarters), and/or a broad search for other `order_frequency == 91` groups turns up ones with more rows | Submitting all N dates over-populates prod (e.g. 6 rows when 3 was correct — TBRCS-757) | Submit **3 rows** (one quarter) — `order_frequency` 91 already signals the pattern repeats every quarter. Verify row count against the **actual matching-cadence** reference group (e.g. the one named in Planner notes / ticket text), not any group that merely shares the same `order_frequency` ([TBRCS-757](#excel-layout-title-row--header-detection)) |
...
| `order_schedule_date` format drift after the Forge Anvil migration — DuckDB reads the datastore CSV and returns whatever literal text is stored, and real prod data has **mixed formats across rows** (older rows `YYYY-MM-DD HH:MM:SS`, others plain `M/D/YYYY`) | `pd.to_datetime()` without `format="mixed"` **crashes** on the first row that doesn't match the format inferred from earlier rows; naively passing the column straight to `.to_csv()` also silently changes the on-disk format (e.g. ISO instead of the original `M/D/YYYY`), which looks like a diff to anyone reviewing the output | `order_sch_main.py` normalizes `old_df["order_schedule_date"]` right after the read via `pd.to_datetime(..., format="mixed")` + the existing `format_schedule_date()` helper, so every row (backup **and** final output) comes out as consistent `M/D/YYYY` — matching the original convention regardless of how it was stored |
| Fiscal pair has a **uniform civil gap** that's neither a 91-multiple nor exactly 28 (e.g. **63-day "every other fiscal month"**), and the submitted `order_frequency` was copy-pasted from a different pair's row (e.g. **91** instead of the correct **182** for a stride-2 pattern) | Merge tool's default expand fallback silently reinterprets the pair as **consecutive** months, duplicating another pair's dates and dropping a submitted row entirely; even if you preserved the literal submitted `order_frequency`, using the wrong value would eventually make the downstream recurring engine order far more often than intended | Verify the submitted dates against `fiscal_cal` directly (they may already be correct); if `order_frequency` doesn't match the stride the dates imply, correct it in `--manual-csv` (91→182 for stride 2, dates unchanged) — don't trust the submitted frequency or the tool's fallback output ([TBRCS-758](#excel-layout-title-row--header-detection)) |
| Ticket/naming describes a cadence with a **nominal** day count (e.g. "every other month (56 days)", `OrdReview_..._56D_...`) | Judging the merge output against that literal number looks like a QA failure when civil gaps come out to 63 (or another value) instead | Fiscal-month lengths vary (4 or 5 weeks), so "56D"/"28D"-style naming is a rounded label, not an exact day-count — same caveat as existing `28D` naming ([Fiscal months vs civil dates](#fiscal-months-vs-civil-dates-454)); validate via `fiscal_cal`, not the naming or ticket-text day count |
